...
function drawPolygon(ctx, poly)
{
  ...
  var len_poly_draw = poly.length; //number of points in the polygon
  for(var i_poly_draw = 0; i_poly_draw < len_poly_draw - 1; i_poly_draw++) //loop through the points
{
    drawLineSegment(ctx, poly[i_poly_draw][0], poly[i_poly_draw][1], poly[i_poly_draw+1][0], poly[i_poly_draw+1][1]); //this draws the lines from first point to ... to last point
}
    drawLineSegment(ctx, poly[len_poly_draw-1][0], poly[len_poly_draw-1][1], poly[0][0], poly[0][1]); //this draws the line from last point to first point
}
...
function transformPolygon(poly, T)
{
  ...
  var len_poly_trans = poly.length; //how many points in the polygon
  for(var i_poly_trans = 0; i_poly_trans < len_poly_trans; i_poly_trans++)
  {
    poly[i_poly_trans] = transformPoint(poly[i_poly_trans], T); //transforming a polygon can be decomposed to transforming each point in it
  } 
  return poly;
}
...
function drawCircle(ctx, cx, cy, r)
{
  /** Fill in your answer here **/
  // Hint: call drawLineSegment to help you
  for(var i_circle_draw = 0; i_circle_draw < 100; i_circle_draw++)
  {
    drawLineSegment(ctx, r*Math.cos(2*3.14159*i_circle_draw/100) + cx, r*Math.sin(2*3.14159*i_circle_draw/100) + cy, r*Math.cos(2*3.14159*(i_circle_draw+1)/100) + cx, r*Math.sin(2*3.14159(i_circle_draw+1)/100) + cy); //dividing a circle into 100 points on the circle evenly,then connecting the points to get the circle (polygon)
  } 
}
...
function translateByOffset(offset)
{
  ...
  /** Fill in your answer here **/
  T[0][2] = offset[0]; //in homogeneous coordinate, the offset value is stored at the right side of the T
  T[1][2] = offset[1]; //in homogeneous coordinate, the offset value is stored at the right side of the T
  return T;
}
...
function rotationAboutPoint(angle, joint)
{
  ...
  T[0][1] = (-1)*Math.sin(angle);
  T[0][2] = -joint[0]*Math.cos(angle)+joint[0]+joint[1]*Math.sin(angle); //https://math.stackexchange.com/questions/2093314/rotation-matrix-and-of-rotation-around-a-point
  T[1][0] = Math.sin(angle);
  T[1][1] = Math.cos(angle);
  T[1][2] = -joint[0]*Math.sin(angle)-joint[1]*Math.cos(angle)+joint[1]; //https://math.stackexchange.com/questions/2093314/rotation-matrix-and-of-rotation-around-a-point
  return T;
  //The matrix for rotation w.r.t. a specific point was found in the above website
}
